  ...
  try {
    const [result] = await pool.query(
      `INSERT INTO products (name, price, image_url, category, description, in_stock)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [name.trim(), Number(price), imageUrl.trim(), category.trim(), description.trim(), inStock ? 1 : 0],
    );
  ...
  try {
    const [updateResult] = await pool.query(
      `UPDATE products
       ...
       WHERE id = ?`,
      [name.trim(), Number(price), imageUrl.trim(), category.trim(), description.trim(), inStock ? 1 : 0, id],
    );
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    if (error && error.code === 'ER_ROW_IS_REFERENCED_2') {
      return sendError(res, 409, 'No se puede eliminar: el producto existe en el carrito');
    }
